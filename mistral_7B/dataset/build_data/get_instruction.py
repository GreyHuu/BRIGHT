...
def get_instruction(current_relation, input, output, sim_input_ouput):
    ...
    if not system_prompt and not relation_prompt:
        return ""
    current = f'<s> [INST] system_prompt: {system_prompt} relation_prompt: {relation_prompt} Relation: {current_relation} Input entity:{input} [/INST] Output entities: {",".join(output)} </s>'
    sim = ""
    for item in sim_input_ouput:
        input = item["input"]
        output = item["output"]
        if isinstance(output, list):
            sim = sim + f'<s> [INST] Relation:{current_relation} Input entity:{input} [/INST] Output entities: {",".join(output)} </s> '
